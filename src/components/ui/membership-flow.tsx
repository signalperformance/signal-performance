...
export function MembershipFlow({ tiers, className, title, subtitle }: MembershipFlowProps) {
  const { t } = useLanguage();
  ...
  const currentTier = useMemo(
    () => orderedTiers.find((x) => x.id === activeTier) ?? orderedTiers[0] ?? tiers[0],
    [activeTier, orderedTiers, tiers]
  );

  const sectionTitle = title ?? t("pricing.title");

  return (
    <section className={cn("section-padding text-foreground", "overflow-hidden", className)}>

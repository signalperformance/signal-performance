...
const WeeklySchedule = () => {
  ...
  type Item = { time: string; labelKey: 'mobility' | 'strength' | 'cardio' | 'power'; pro?: boolean };

  const mwf: Item[] = [
    { time: `12 ${t('schedule.timePeriods.pm')}`, labelKey: 'strength', pro: true },
    { time: `1 ${t('schedule.timePeriods.pm')}`, labelKey: 'strength', pro: true },
    { time: `2 ${t('schedule.timePeriods.pm')}`, labelKey: 'strength', pro: true },
    { time: `5 ${t('schedule.timePeriods.pm')}`, labelKey: 'mobility' },
    { time: `6 ${t('schedule.timePeriods.pm')}`, labelKey: 'strength' },
    { time: `7 ${t('schedule.timePeriods.pm')}`, labelKey: 'strength' },
  ];

  const tth: Item[] = [
    { time: `12 ${t('schedule.timePeriods.pm')}`, labelKey: 'power', pro: true },
    { time: `1 ${t('schedule.timePeriods.pm')}`, labelKey: 'power', pro: true },
    { time: `2 ${t('schedule.timePeriods.pm')}`, labelKey: 'power', pro: true },
    { time: `5 ${t('schedule.timePeriods.pm')}`, labelKey: 'power' },
    { time: `6 ${t('schedule.timePeriods.pm')}`, labelKey: 'cardio' },
    { time: `7 ${t('schedule.timePeriods.pm')}`, labelKey: 'power' },
  ];

  const weekend: Item[] = [
    { time: `9 ${t('schedule.timePeriods.am')}`, labelKey: 'mobility' },
    { time: `10 ${t('schedule.timePeriods.am')}`, labelKey: 'strength' },
    { time: `11 ${t('schedule.timePeriods.am')}`, labelKey: 'cardio' },
    { time: `12 ${t('schedule.timePeriods.pm')}`, labelKey: 'power' },
  ];
              ...
              <Card className="overflow-hidden border-border/70 shadow-sm hover:shadow-md transition-shadow">
                ...
                <CardContent className="pt-0">
                  <ul className="space-y-2" role="list">
                    {col.items.map((it, i) => (
                      <li key={i} className={`relative flex items-center justify-between rounded-md border border-border/60 px-3 py-2 ${it.pro ? 'bg-signal-gold/10' : 'bg-card/50'}`}>
